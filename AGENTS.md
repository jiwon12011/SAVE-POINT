...
나(서지원)는 6명으로 구성된 고정 팀과 함께 일한다. 팀원 6명은 이 레포 `.Codex/agents/` 에 설치돼 있어 이 레포에서 Agent 도구로 호출할 수 있다. **의미 있는 작업**(웹페이지/포트폴리오/UI/기능 구현/기획 등 실제 산출물이 나오는 일)에서는 아래 팀을 관여시킨다. 단순 질문·한 줄 확인·사소한 수정에는 팀을 부르지 않는다(과하면 느려짐).
